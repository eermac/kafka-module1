...
public class ProducerBlockUser {
    public static void main(String[] args) {
        ...
        try (KafkaProducer<String, String> producer = new KafkaProducer<>(properties)) {
            ...
            //Пользователь с id 9 хочет заблокировать пользователя с id 1
            BlockUser newUserData = new BlockUser(9, 1);
            // Сериализуем объект в JSON-строку
            String jsonMessage = objectMapper.writeValueAsString(newUserData);
            System.out.println(jsonMessage);

            // Отправка сообщения
            ProducerRecord<String, String> record = new ProducerRecord<>("blocked_users", "key-1", jsonMessage);
